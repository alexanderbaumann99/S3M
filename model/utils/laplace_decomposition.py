...
def cotangent_laplacian(verts, faces):
    ...
    Cos1 = (L2**2 + L3**2 - L1**2) / (2 * L2 * L3 + 1e-5)
    Cos2 = (L1**2 + L3**2 - L2**2) / (2 * L1 * L3 + 1e-5)
    Cos3 = (L1**2 + L2**2 - L3**2) / (2 * L1 * L2 + 1e-5)
    Cos = np.column_stack((Cos1, Cos2, Cos3))
    ...
    In = np.arange(nv)
    A = csr_matrix(
        (np.squeeze(np.array(Area.todense())), (In, In)), [nv, nv]
